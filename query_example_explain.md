...
本文档描述的是**阶段2：实时推荐（用户查询）**，不涉及特征生成阶段。

| 阶段 | 名称 | 触发时机 | 本文档是否涵盖 |
|------|------|----------|----------------|
| **阶段1** | 特征生成（离线准备） | 管理员手动触发或定时任务 | ❌ 不涵盖 |
| **阶段2** | 实时推荐（用户查询） | 用户发送 query 时 | ✅ 本文档 |

**阶段1**（特征生成）的详细说明请参阅 `design.md` 中的 **3.0.5 两阶段数据流说明** 和 **3.0.6 预聚合表生成操作指南**。
...
阶段1（特征生成）                          阶段2（实时推荐）
───────────────────                        ───────────────────
songs + song_playlist_agg                  music_features
     + song_comment_agg  ────────┐              │
     ↓                           │              ↓
LLM 提取特征                         │    用户 query
     ↓                           │        │
...
**涉及的表**: `music_features`（特征表）、`songs`（歌曲表）

**优化说明**：
> 严格来说，实时查询应避免 JOIN。但此处需要 `songs` 表中的 `song_name`、`artist`、`album`、`music_file` 字段用于：
> - 显示给用户的歌曲信息
> - 提供给播放器播放的音频文件路径
>
> **优化方案**：可以将这些字段直接存储在 `music_features` 表中，彻底消除实时查询的 JOIN。设计文档中建议的 `music_features` 表已包含这些字段。
