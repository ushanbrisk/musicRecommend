# 第6部分：AI搜索推荐实现步骤（最新版本）

> **当前状态**：前端和后端代码已完成，现在需要实现两阶段AI搜索推荐功能
>
> **目标**：基于两阶段架构（向量召回 + LLM精排）实现智能音乐推荐

---

## 实现路线图

```
┌─────────────────────────────────────────────────────────────────┐
│                      实现路线图                                   │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  阶段1：向量数据库准备 ✅ 必须                                      │
│         ├─ 安装 pgvector 扩展                                    │
│         ├─ 创建 song_embeddings 表                               │
│         └─ 生成歌曲向量数据                                       │
│                                                                  │
│  阶段2：Embedding 服务 ✅ 必须                                     │
│         ├─ 在 llm_service.py 添加 embedding 方法                 │
│         └─ 测试 embedding API 连通性                             │
│                                                                  │
│  阶段3：推荐服务实现 ✅ 核心                                        │
│         ├─ 实现向量召回逻辑（阶段1）                               │
│         ├─ 实现 LLM 精排逻辑（阶段2）                              │
│         └─ 整合两阶段推荐流程                                     │
│                                                                  │
│  阶段4：API 测试与调优 ✅ 必须                                     │
│         ├─ 端到端测试推荐接口                                     │
│         ├─ 性能测试（响应时间）                                   │
│         └─ 结果质量验证                                          │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

---

## 阶段1：向量数据库准备（预计工作量：1-2天）

### 1.1 安装 pgvector 扩展

**目标**：在 PostgreSQL 中启用向量检索能力

```bash
# 1. 连接到数据库
PGPASSWORD=luke psql -h localhost -U postgres -d musicdb

# 2. 启用 pgvector 扩展
CREATE EXTENSION IF NOT EXISTS vector;

# 3. 验证扩展已安装
\dx vector
```

**预期输出**：
```
                          List of installed extensions
  Name   | Version |   Schema   |                Description
---------+---------+------------+--------------------------------------------
 vector  | 0.5.0   | public     | vector data type and ivfflat access method
```

**如果扩展不存在，需要先安装**：
```bash
# Ubuntu/Debian
#需要修改源，用阿里源，本地的Postgresql 需要升级到13
sudo apt install postgresql-13-pgvector

# 或从源码编译
cd /tmp
git clone --branch v0.5.0 https://github.com/pgvector/pgvector.git
cd pgvector
make
sudo make install
```

---

### 1.2 创建 song_embeddings 表

**目标**：创建专门存储歌曲向量的表

```bash
PGPASSWORD=luke psql -h localhost -U postgres -d musicdb
```

```sql
-- 创建歌曲向量表
CREATE TABLE IF NOT EXISTS song_embeddings (
    id SERIAL PRIMARY KEY,
    song_id BIGINT UNIQUE NOT NULL REFERENCES songs(song_id),

    -- 文本描述（用于生成向量和调试）
    text_description TEXT NOT NULL,

    -- 向量表示（1536维，与 Embedding 模型对齐）
    embedding vector(1536),

    -- 元数据（用于过滤和辅助排序）
    genre VARCHAR(100),
    mood VARCHAR(200),
    scene VARCHAR(200),
    language VARCHAR(50),

    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);

-- 创建 HNSW 索引（适合追求查询速度的场景，召回阶段首选）
CREATE INDEX IF NOT EXISTS idx_song_embeddings_hnsw
ON song_embeddings USING hnsw (embedding vector_cosine_ops);

-- 索引设计：元数据索引，支持混合检索（向量检索 + 标签过滤）
CREATE INDEX IF NOT EXISTS idx_song_embeddings_genre ON song_embeddings(genre);
CREATE INDEX IF NOT EXISTS idx_song_embeddings_mood ON song_embeddings(mood);
CREATE INDEX IF NOT EXISTS idx_song_embeddings_scene ON song_embeddings(scene);
CREATE INDEX IF NOT EXISTS idx_song_embeddings_song_id ON song_embeddings(song_id);
```

**验证表创建成功**：
```sql
\d song_embeddings
SELECT COUNT(*) FROM song_embeddings;  -- 应该返回 0（初始为空）
```
上面的创建表语句已经添加进database/schema.sql中
可以一键执行
```
psql -h localhost -U postgres -d musicdb -f /home/luke/code_project/musicRecommend/database/schema.sql
```
---

### 1.3 生成歌曲向量数据

**目标**：为所有歌曲生成 Embedding 向量并写入 song_embeddings 表

#### 步骤1：创建向量生成脚本

创建文件 `/home/luke/code_project/musicRecommend/scripts/init_song_embeddings.py`

```python
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
歌曲 Embedding 向量生成脚本
功能：从 songs 表读取数据，生成向量，写入 song_embeddings 表
"""

import os
import sys
import psycopg2
from openai import OpenAI
from dotenv import load_dotenv
import time

# 加载环境变量
backend_env = os.path.expanduser('~/code_project/music-project/backend/.env')
load_dotenv(backend_env)

# 初始化 Embedding 客户端
client = OpenAI(
    api_key=os.getenv('EMBEDDING_API_KEY'),
    base_url=os.getenv('EMBEDDING_PROVIDER_URL', 'https://api.minimax.chat')
)
EMBEDDING_MODEL = os.getenv('EMBEDDING_MODEL_NAME', 'embo-01')

# 数据库连接
def get_db_connection():
    return psycopg2.connect(
        host=os.getenv('POSTGRES_HOST', 'localhost'),
        port=os.getenv('POSTGRES_PORT', 5432),
        database=os.getenv('POSTGRES_DB', 'musicdb'),
        user=os.getenv('POSTGRES_USER', 'postgres'),
        password=os.getenv('POSTGRES_PASSWORD', '')
    )

def generate_embedding(text: str):
    """调用 Embedding API 生成向量"""
    try:
        response = client.embeddings.create(
            model=EMBEDDING_MODEL,
            input=text
        )
        return response.data[0].embedding
    except Exception as e:
        print(f"Error generating embedding: {e}")
        return None

def get_songs_without_embeddings(conn, limit=1000):
    """获取尚未生成向量的歌曲"""
    cursor = conn.cursor()
    query = """
        SELECT
            s.song_id,
            s.song_name,
            s.artist,
            s.album,
            spa.playlist_names,
            spa.playlist_categories
        FROM songs s
        LEFT JOIN song_embeddings se ON s.song_id = se.song_id
        LEFT JOIN song_playlist_agg spa ON s.song_id = spa.song_id
        WHERE se.id IS NULL
        LIMIT %s
    """
    cursor.execute(query, (limit,))
    rows = cursor.fetchall()
    cursor.close()

    songs = []
    for row in rows:
        songs.append({
            'song_id': row[0],
            'song_name': row[1],
            'artist': row[2],
            'album': row[3],
            'playlist_names': row[4] or '',
            'playlist_categories': row[5] or ''
        })
    return songs

def build_text_description(song):
    """构建歌曲的文本描述（用于生成 embedding）"""
    parts = []

    # 歌曲名
    if song['song_name']:
        parts.append(song['song_name'])

    # 艺术家
    if song['artist']:
        parts.append(song['artist'])

    # 专辑
    if song['album']:
        parts.append(song['album'])

    # 歌单名称（最多取5个）
    if song['playlist_names']:
        playlists = song['playlist_names'].split(',')[:5]
        parts.extend(playlists)

    # 歌单分类
    if song['playlist_categories']:
        categories = song['playlist_categories'].split(',')[:3]
        parts.extend(categories)

    return ' '.join(parts)

> **文本描述构建说明**：只用歌名+艺术家+专辑+歌单信息，暂不加入评论数据。原因：评论内容较杂乱（"太好听了"、"一般般"），不适合作为语义特征；歌单名称/分类（如"欧美流行"、"欢快"）更能反映歌曲风格/情绪/场景等语义信息。

def save_embedding(conn, song_id, text_description, embedding_vector):
    """保存向量到数据库"""
    cursor = conn.cursor()
    query = """
        INSERT INTO song_embeddings (
            song_id, text_description, embedding
        ) VALUES (%s, %s, %s)
        ON CONFLICT (song_id) DO UPDATE SET
            text_description = EXCLUDED.text_description,
            embedding = EXCLUDED.embedding,
            updated_at = CURRENT_TIMESTAMP
    """
    cursor.execute(query, (song_id, text_description, embedding_vector))
    conn.commit()
    cursor.close()

def main():
    print("开始生成歌曲 Embedding 向量...")

    conn = get_db_connection()

    # 统计总数
    cursor = conn.cursor()
    cursor.execute("SELECT COUNT(*) FROM songs")
    total_songs = cursor.fetchone()[0]
    cursor.execute("SELECT COUNT(*) FROM song_embeddings")
    existing_count = cursor.fetchone()[0]
    cursor.close()

    print(f"歌曲总数: {total_songs}")
    print(f"已生成向量: {existing_count}")
    print(f"待生成: {total_songs - existing_count}")

    batch_size = 1000
    processed = 0

    while True:
        songs = get_songs_without_embeddings(conn, limit=batch_size)
        if not songs:
            print("所有歌曲向量已生成完毕！")
            break

        print(f"\n批次处理: {len(songs)} 首歌曲")

        for i, song in enumerate(songs, 1):
            song_id = song['song_id']
            text_desc = build_text_description(song)

            # 生成向量
            embedding = generate_embedding(text_desc)
            if embedding is None:
                print(f"  [{i}/{len(songs)}] Song {song_id}: 向量生成失败，跳过")
                continue

            # 保存到数据库
            save_embedding(conn, song_id, text_desc, embedding)
            processed += 1

            if i % 100 == 0:
                print(f"  [{i}/{len(songs)}] 已处理 {processed} 首")

            # 避免 API 限流
            time.sleep(0.05)

        print(f"批次完成，累计处理: {processed} 首")

    conn.close()
    print(f"\n总计生成向量: {processed} 首")

if __name__ == '__main__':
    main()
```

#### 步骤2：配置环境变量

编辑 `/home/luke/code_project/music-project/backend/.env`，添加：

```bash
# Embedding API 配置
EMBEDDING_PROVIDER_URL=https://api.minimax.chat
EMBEDDING_API_KEY=your_embedding_api_key
EMBEDDING_MODEL_NAME=embo-01
```

#### 步骤3：执行向量生成

**方案 A：使用 API（默认）**
```bash
cd /home/luke/code_project/musicRecommend
~/miniconda3/envs/music_recommend/bin/python scripts/init_song_embeddings.py
```

**方案 B：使用本地 GPU（推荐，已缓存模型时）**
```bash
cd /home/luke/code_project/musicRecommend
# 注意：如果本地已有模型缓存，需要设置 HF_HUB_OFFLINE=1 避免联网验证
HF_HUB_OFFLINE=1 EMBEDDING_USE_LOCAL=true GPU_GROUP=0 ~/miniconda3/envs/music_recommend/bin/python scripts/init_song_embeddings.py
```

**方案 B：多卡并行（6 张卡同时跑）**
```bash
cd /home/luke/code_project/musicRecommend
# 同时跑 6 个卡（需要先设置 HF_HUB_OFFLINE=1）
HF_HUB_OFFLINE=1 EMBEDDING_USE_LOCAL=true GPU_GROUP=0 python scripts/init_song_embeddings.py &
HF_HUB_OFFLINE=1 EMBEDDING_USE_LOCAL=true GPU_GROUP=1 python scripts/init_song_embeddings.py &
HF_HUB_OFFLINE=1 EMBEDDING_USE_LOCAL=true GPU_GROUP=2 python scripts/init_song_embeddings.py &
HF_HUB_OFFLINE=1 EMBEDDING_USE_LOCAL=true GPU_GROUP=3 python scripts/init_song_embeddings.py &
HF_HUB_OFFLINE=1 EMBEDDING_USE_LOCAL=true GPU_GROUP=4 python scripts/init_song_embeddings.py &
HF_HUB_OFFLINE=1 EMBEDDING_USE_LOCAL=true GPU_GROUP=5 python scripts/init_song_embeddings.py
```

**预计耗时**：
- 每首歌约 0.1 秒（本地 GPU）
- 100万首歌约 27 小时（单卡，可分批执行）
- 约 4-5 小时（6 卡并行）
- 建议先生成有歌单的12万首歌（约 3-4 小时）

#### 步骤4：验证向量生成

```bash
PGPASSWORD=luke psql -h localhost -U postgres -d musicdb -c "
SELECT COUNT(*) as total,
       COUNT(embedding) as with_vector,
       COUNT(*) - COUNT(embedding) as without_vector
FROM song_embeddings;
"
```

---

## 阶段2：Embedding 服务（预计工作量：0.5天）

### 2.1 在 llm_service.py 添加 embedding 方法

**目标**：在 LLM 服务中添加 Embedding 功能，支持 API 和本地模型两种方式

编辑 `/home/luke/code_project/music-project/backend/services/llm_service.py`

```python
# backend/services/llm_service.py
"""
LLM 服务模块

支持三种 Embedding 生成方式：
    - use_local=False → API 方式（默认）
    - use_local=True + 有 GPU → 本地 GPU（使用 cuda:{gpu_group}）
    - use_local=True + 无 GPU → 本地 CPU

环境变量配置：
    # API 方式（默认）
    EMBEDDING_API_KEY=xxx
    EMBEDDING_PROVIDER_URL=https://api.minimax.chat
    EMBEDDING_MODEL_NAME=embo-01

    # 本地模型方式
    EMBEDDING_USE_LOCAL=true
    GPU_GROUP=0                    # GPU 编号，0-5
    LOCAL_EMBEDDING_MODEL=BAAI/bge-m3  # 本地模型名称

使用示例：
    llm = LLMService()
    embedding = llm.generate_embedding("欢快的凯尔特音乐")
"""

import os
from openai import OpenAI
from typing import List, Dict, Optional

class LLMService:
    def __init__(self):
        # Chat 客户端
        self.chat_client = OpenAI(
            api_key=os.getenv("LLM_PROVIDER_KEY"),
            base_url=os.getenv("LLM_PROVIDER_URL")
        )
        self.chat_model = os.getenv("LLM_MODEL_NAME", "abab6.5s-chat")

        # Embedding 配置
        self.use_local = os.getenv('EMBEDDING_USE_LOCAL', 'false').lower() == 'true'
        self.gpu_group = int(os.getenv('GPU_GROUP', '0'))

        # 初始化 embedding 客户端/模型
        self.embedding_client = None
        self.local_model = None
        self.local_tokenizer = None
        self.local_device = None

        if self.use_local:
            self._setup_local_model()
        else:
            self._setup_api_client()

    def _setup_api_client(self):
        """初始化 Embedding API 客户端"""
        self.embedding_client = OpenAI(
            api_key=os.getenv('EMBEDDING_API_KEY'),
            base_url=os.getenv('EMBEDDING_PROVIDER_URL')
        )
        self.embedding_model = os.getenv('EMBEDDING_MODEL_NAME', 'embo-01')

    def _setup_local_model(self):
        """初始化本地 Embedding 模型"""
        import torch
        from transformers import AutoModel, AutoTokenizer

        device = f"cuda:{self.gpu_group}" if torch.cuda.is_available() else "cpu"
        model_name = os.getenv('LOCAL_EMBEDDING_MODEL', 'BAAI/bge-m3')

        print(f"Loading local embedding model: {model_name}")
        print(f"Using device: {device}")

        self.local_tokenizer = AutoTokenizer.from_pretrained(model_name, trust_remote_code=True)
        self.local_model = AutoModel.from_pretrained(model_name, trust_remote_code=True)
        self.local_model.to(device)
        self.local_model.eval()

        self.local_device = device

    def _mean_pooling(self, model_output, attention_mask):
        """Mean Pooling - 考虑 attention mask 的平均池化"""
        import torch
        token_embeddings = model_output[0]
        input_mask_expanded = attention_mask.unsqueeze(-1).expand(token_embeddings.size()).float()
        return torch.sum(token_embeddings * input_mask_expanded, 1) / torch.clamp(input_mask_expanded.sum(1), min=1e-9)

    def generate_embedding_api(self, text: str) -> Optional[List[float]]:
        """调用 Embedding API 生成向量"""
        try:
            response = self.embedding_client.embeddings.create(
                model=self.embedding_model,
                input=text,
                encoding_format="float"
            )
            return response.data[0].embedding
        except Exception as e:
            print(f"Embedding API error: {e}")
            return None

    def generate_embedding_local(self, text: str) -> Optional[List[float]]:
        """使用本地模型生成向量"""
        import torch
        try:
            encoded_input = self.local_tokenizer(
                text, padding=True, truncation=True, max_length=1024, return_tensors='pt'
            )
            encoded_input = {k: v.to(self.local_device) for k, v in encoded_input.items()}

            with torch.no_grad():
                model_output = self.local_model(**encoded_input)

            embedding = self._mean_pooling(model_output, encoded_input['attention_mask'])
            # L2 归一化
            embedding = torch.nn.functional.normalize(embedding, p=2, dim=1)

            return embedding[0].cpu().numpy().tolist()
        except Exception as e:
            print(f"Local embedding error: {e}")
            return None

    def generate_embedding(self, text: str) -> Optional[List[float]]:
        """生成文本 embedding 向量（统一接口）"""
        if self.use_local:
            return self.generate_embedding_local(text)
        else:
            return self.generate_embedding_api(text)
```

### 2.2 测试 Embedding

创建测试脚本 `/home/luke/code_project/musicRecommend/tests/test_embedding.py`

```python
#!/usr/bin/env python3
"""
Embedding 测试脚本

用法:
    # 单模型测试（使用当前环境变量配置）
    ~/miniconda3/envs/music/bin/python tests/test_embedding.py

    # API 模式测试（强制使用 API）
    EMBEDDING_USE_LOCAL=false ~/miniconda3/envs/music/bin/python tests/test_embedding.py --api-only

    # 本地模式测试（强制使用本地）
    EMBEDDING_USE_LOCAL=true ~/miniconda3/envs/music/bin/python tests/test_embedding.py --local-only

    # 一致性测试（比较 API 和本地模型）
    ~/miniconda3/envs/music/bin/python tests/test_embedding.py --consistency
"""

import sys
import os
import subprocess
import tempfile
import json
from pathlib import Path

sys.path.insert(0, os.path.expanduser('~/code_project/music-project/backend'))

from services.llm_service import LLMService
from dotenv import load_dotenv
import numpy as np

# 加载环境变量
backend_env = os.path.expanduser('~/code_project/music-project/backend/.env')
load_dotenv(backend_env)


def cosine_similarity(a, b):
    """计算余弦相似度"""
    return np.dot(a, b) / (np.linalg.norm(a) * np.linalg.norm(b))


def vector_norm(v):
    """计算向量的 L2 范数"""
    return np.linalg.norm(v)


def test_embedding():
    """测试单一 embedding 生成"""
    print("\n" + "=" * 60)
    print("测试 Embedding 生成")
    print("=" * 60)

    llm = LLMService()
    mode = "本地模型" if llm.use_local else "API"
    print(f"当前模式: {mode}")

    test_text = "欢快的凯尔特音乐，有小提琴伴奏"
    print(f"测试文本: {test_text}")

    embedding = llm.generate_embedding(test_text)

    if embedding:
        print(f"✅ Embedding 生成成功")
        print(f"   向量维度: {len(embedding)}")
        print(f"   前5个值: {embedding[:5]}")
        print(f"   L2 范数: {vector_norm(embedding):.6f}")
        return True
    else:
        print("❌ Embedding 生成失败")
        return False


def test_api_vs_local_consistency():
    """测试 API 和本地模型生成的向量一致性（使用子进程隔离）"""
    print("\n" + "=" * 60)
    print("测试 API 与本地模型 Embedding 一致性")
    print("=" * 60)

    test_texts = [
        "欢快的凯尔特音乐，有小提琴伴奏",
        "安静的钢琴曲，适合冥想",
        "激烈的重金属摇滚",
        "温柔的爵士乐",
    ]

    api_embeddings = []
    local_embeddings = []

    python_path = str(Path.home() / "miniconda3/envs/music/bin/python")

    # 测试脚本模板 - 直接设置环境变量，不使用 dotenv
    test_script_template = '''
import sys
import os
sys.path.insert(0, '/home/luke/code_project/music-project/backend')

# 设置环境变量（不依赖 .env 文件）
os.environ['EMBEDDING_USE_LOCAL'] = '{use_local}'
os.environ['EMBEDDING_API_KEY'] = '{api_key}'
os.environ['EMBEDDING_PROVIDER_URL'] = '{provider_url}'
os.environ['EMBEDDING_MODEL_NAME'] = '{model_name}'
os.environ['LLM_PROVIDER_KEY'] = '{llm_key}'
os.environ['LLM_PROVIDER_URL'] = '{llm_url}'

from services.llm_service import LLMService
import json

llm = LLMService()
texts = {texts}
for text in texts:
    embedding = llm.generate_embedding(text)
    print(json.dumps({{'text': text, 'embedding': embedding}}))
'''

    # 从当前环境获取配置
    api_key = os.getenv('EMBEDDING_API_KEY', '')
    provider_url = os.getenv('EMBEDDING_PROVIDER_URL', '')
    model_name = os.getenv('EMBEDDING_MODEL_NAME', 'BAAI/bge-m3')
    llm_key = os.getenv('LLM_PROVIDER_KEY', '')
    llm_url = os.getenv('LLM_PROVIDER_URL', '')

    # 生成 API embeddings
    print("\n[1/2] 生成 API embedding...")
    test_script = test_script_template.format(
        use_local='false',
        api_key=api_key,
        provider_url=provider_url,
        model_name=model_name,
        llm_key=llm_key,
        llm_url=llm_url,
        texts=str(test_texts)
    )

    with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as f:
        f.write(test_script)
        api_script_path = f.name

    try:
        result = subprocess.run(
            [python_path, api_script_path],
            capture_output=True, text=True, timeout=300, cwd='/home/luke/code_project/musicRecommend'
        )
        if result.returncode == 0:
            for line in result.stdout.strip().split('\n'):
                if line and not line.startswith('Loading'):
                    try:
                        data = json.loads(line)
                        api_embeddings.append(data['embedding'])
                        print(f"  ✅ API embedding 生成成功")
                    except json.JSONDecodeError:
                        pass
        else:
            print(f"  ❌ API 测试失败: {result.stderr[:500]}")
    except Exception as e:
        print(f"  ❌ API 测试异常: {e}")
    finally:
        os.unlink(api_script_path)

    # 生成本地 embeddings
    print("\n[2/2] 生成本地 embedding...")
    test_script = test_script_template.format(
        use_local='true',
        api_key=api_key,
        provider_url=provider_url,
        model_name=model_name,
        llm_key=llm_key,
        llm_url=llm_url,
        texts=str(test_texts)
    )

    with tempfile.NamedTemporaryFile(mode='w', suffix='.py', delete=False) as f:
        f.write(test_script)
        local_script_path = f.name

    try:
        result = subprocess.run(
            [python_path, local_script_path],
            capture_output=True, text=True, timeout=300, cwd='/home/luke/code_project/musicRecommend',
            env={**os.environ, 'PYTHONUNBUFFERED': '1'}
        )
        if result.returncode == 0:
            for line in result.stdout.strip().split('\n'):
                if line and not line.startswith('Loading') and not line.startswith('Using'):
                    try:
                        data = json.loads(line)
                        local_embeddings.append(data['embedding'])
                        print(f"  ✅ 本地 embedding 生成成功")
                    except json.JSONDecodeError:
                        pass
        else:
            print(f"  ❌ 本地测试失败: {result.stderr[:500]}")
    except Exception as e:
        print(f"  ❌ 本地测试异常: {e}")
    finally:
        os.unlink(local_script_path)

    # 计算相似度
    print("\n" + "=" * 60)
    print("向量相似度对比结果")
    print("=" * 60)

    if len(api_embeddings) == 0 or len(local_embeddings) == 0:
        print("❌ 无法完成对比测试（embedding 生成失败）")
        return False

    all_passed = True
    api_norms = []
    local_norms = []

    for i, text in enumerate(test_texts):
        if i >= len(api_embeddings) or i >= len(local_embeddings):
            continue

        api_vec = np.array(api_embeddings[i])
        local_vec = np.array(local_embeddings[i])

        # 计算 L2 范数
        api_norm = vector_norm(api_vec)
        local_norm = vector_norm(local_vec)
        api_norms.append(api_norm)
        local_norms.append(local_norm)

        # 余弦相似度
        cos_sim = cosine_similarity(api_vec, local_vec)

        # 欧氏距离
        euclidean_dist = np.linalg.norm(api_vec - local_vec)

        # 判断是否通过（余弦相似度 > 0.99 视为通过）
        passed = cos_sim > 0.99

        if not passed:
            all_passed = False

        status = "✅ 通过" if passed else "⚠️  注意"
        print(f"\n文本: {text}")
        print(f"  API 向量 L2 范数: {api_norm:.6f}")
        print(f"  本地向量 L2 范数: {local_norm:.6f}")
        print(f"  余弦相似度: {cos_sim:.6f} (阈值 > 0.99) {status}")
        print(f"  欧氏距离: {euclidean_dist:.6f}")

    print("\n" + "=" * 60)
    print("分析结论")
    print("=" * 60)

    avg_api_norm = np.mean(api_norms)
    avg_local_norm = np.mean(local_norms)

    print(f"API 向量平均 L2 范数: {avg_api_norm:.6f}")
    print(f"本地向量平均 L2 范数: {avg_local_norm:.6f}")

    if all_passed:
        print("\n✅ 所有测试通过！API 和本地模型生成的向量基本一致")
    else:
        print("\n⚠️  API 和本地模型生成的向量存在差异")
        print("   可能原因：")
        print("   1. API 服务提供商（如 SiliconFlow）对模型做了定制化修改")
        print("   2. API 服务可能没有做 L2 归一化，而本地代码做了归一化")
        print("   3. 模型版本或配置可能略有不同")
        print("")
        print("   如果 API 和本地模型使用相同的模型（如都是 BAAI/bge-m3），")
        print("   但向量差异较大，建议：")
        print("   - 确认 API 提供商使用的是相同的模型版本")
        print("   - 或者在使用时对 API 返回的向量也进行 L2 归一化")

    print("=" * 60)

    return all_passed


if __name__ == '__main__':
    if '--consistency' in sys.argv:
        test_api_vs_local_consistency()
    elif '--api-only' in sys.argv:
        os.environ['EMBEDDING_USE_LOCAL'] = 'false'
        load_dotenv(backend_env, override=True)
        test_embedding()
    elif '--local-only' in sys.argv:
        os.environ['EMBEDDING_USE_LOCAL'] = 'true'
        load_dotenv(backend_env, override=True)
        test_embedding()
    else:
        test_embedding()
```

运行测试：
```bash
# 单模型测试（使用当前环境变量配置）
~/miniconda3/envs/music/bin/python /home/luke/code_project/musicRecommend/tests/test_embedding.py

# API 方式测试
EMBEDDING_USE_LOCAL=false ~/miniconda3/envs/music/bin/python /home/luke/code_project/musicRecommend/tests/test_embedding.py --api-only

# 本地 GPU 方式测试（需要设置 HF_HUB_OFFLINE=1 避免联网验证）
HF_HUB_OFFLINE=1 EMBEDDING_USE_LOCAL=true GPU_GROUP=0 ~/miniconda3/envs/music/bin/python /home/luke/code_project/musicRecommend/tests/test_embedding.py --local-only

# 一致性测试（比较 API 和本地模型）
~/miniconda3/envs/music/bin/python /home/luke/code_project/musicRecommend/tests/test_embedding.py --consistency
```

### 2.3 一致性测试结果与分析

运行一致性测试后，结果如下：

```
============================================================
测试 API 与本地模型 Embedding 一致性
============================================================

[1/2] 生成 API embedding...
  ✅ API embedding 生成成功
  ✅ API embedding 生成成功
  ✅ API embedding 生成成功
  ✅ API embedding 生成成功

[2/2] 生成本地 embedding...
  ✅ 本地 embedding 生成成功
  ✅ 本地 embedding 生成成功
  ✅ 本地 embedding 生成成功
  ✅ 本地 embedding 生成成功

============================================================
向量相似度对比结果
============================================================

文本: 欢快的凯尔特音乐，有小提琴伴奏
  API 向量 L2 范数: 1.000000
  本地向量 L2 范数: 1.000000
  余弦相似度: 0.796422 (阈值 > 0.99) ⚠️  注意
  欧氏距离: 0.638088

文本: 安静的钢琴曲，适合冥想
  API 向量 L2 范数: 1.000000
  本地向量 L2 范数: 1.000000
  余弦相似度: 0.833954 (阈值 > 0.99) ⚠️  注意
  欧氏距离: 0.576275

...

============================================================
分析结论
============================================================
API 向量平均 L2 范数: 1.000000
本地向量平均 L2 范数: 1.000000

⚠️  API 和本地模型生成的向量存在差异
   可能原因：
   1. API 服务提供商（如 SiliconFlow）对模型做了定制化修改
   2. API 服务可能没有做 L2 归一化，而本地代码做了归一化
   3. 模型版本或配置可能略有不同

   如果 API 和本地模型使用相同的模型（如都是 BAAI/bge-m3），
   但向量差异较大，建议：
   - 确认 API 提供商使用的是相同的模型版本
   - 或者在使用时对 API 返回的向量也进行 L2 归一化
============================================================
```

**关键发现：**

1. **L2 范数一致**：API 和本地向量的 L2 范数都是 1.0，说明都已归一化
2. **余弦相似度偏低**：只有 0.79-0.85，表明 API 和本地模型输出的向量存在显著差异

**结论：**

API 服务商（如 SiliconFlow）可能使用了不同版本的模型或做了定制化修改。这是常见情况，不影响代码正确性。

**建议：**

- 离线批量生成和在线查询**必须使用相同的 Embedding 方式**（要么都用本地模型，要么都用 API）
- 混用会导致向量空间不一致，降低召回质量
- 如果使用 API 服务，建议确认 API 提供商使用的模型版本与本地一致

---

## 阶段3：推荐服务实现（预计工作量：2-3天）

### 3.1 实现 RecommendService 的两阶段推荐

**目标**：实现向量召回 + LLM精排的完整流程

编辑 `/home/luke/code_project/music-project/backend/services/recommend_service.py`

```python
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
推荐服务：两阶段推荐算法
- 阶段1：向量数据库粗召回（top 200）
- 阶段2：LLM 精排序（top 20）
"""

import psycopg2
from typing import List, Dict, Optional
import json
import time
from .llm_service import LLMService

class RecommendService:
    def __init__(self, db_connection):
        self.db = db_connection
        self.llm = LLMService()

    def recommend(
        self,
        query: str,
        session_id: Optional[str] = None,
        user_id: Optional[str] = None,
        max_results: int = 20
    ) -> Dict:
        """
        两阶段推荐主流程
        """
        start_time = time.time()

        # 阶段1：向量召回
        candidates = self._vector_recall(query, recall_size=200)

        if not candidates:
            return {
                "success": True,
                "query": query,
                "results": [],
                "total": 0,
                "latency_ms": int((time.time() - start_time) * 1000),
                "message": "未找到匹配的歌曲"
            }

        # 阶段2：LLM 精排
        ranked_results = self._llm_rerank(query, candidates, top_k=max_results)

        # 记录历史
        history_id = self._save_history(
            session_id=session_id,
            user_id=user_id,
            query_text=query,
            result_count=len(ranked_results),
            latency_ms=int((time.time() - start_time) * 1000)
        )

        return {
            "success": True,
            "query": query,
            "history_id": history_id,
            "results": ranked_results,
            "total": len(ranked_results),
            "latency_ms": int((time.time() - start_time) * 1000)
        }

    def _vector_recall(self, query: str, recall_size: int = 200) -> List[Dict]:
        """
        阶段1：向量召回
        使用 pgvector 进行相似度检索
        """
        # 1. 生成查询向量
        query_embedding = self.llm.generate_embedding(query)
        if not query_embedding:
            print("查询向量生成失败")
            return []

        # 2. 向量检索
        cursor = self.db.cursor()

        # 将向量转为字符串格式
        embedding_str = '[' + ','.join(map(str, query_embedding)) + ']'

        query_sql = """
            SELECT
                se.song_id,
                s.song_name,
                s.artist,
                s.album,
                se.text_description,
                se.genre,
                se.mood,
                se.scene,
                1 - (se.embedding <=> %s::vector) AS similarity
            FROM song_embeddings se
            JOIN songs s ON se.song_id = s.song_id
            WHERE se.embedding IS NOT NULL
            ORDER BY se.embedding <=> %s::vector
            LIMIT %s
        """

        cursor.execute(query_sql, (embedding_str, embedding_str, recall_size))
        rows = cursor.fetchall()
        cursor.close()

        candidates = []
        for row in rows:
            candidates.append({
                'song_id': row[0],
                'song_name': row[1],
                'artist': row[2],
                'album': row[3],
                'text_description': row[4],
                'genre': row[5],
                'mood': row[6],
                'scene': row[7],
                'similarity': float(row[8])
            })

        print(f"向量召回: {len(candidates)} 首候选歌曲")
        return candidates

    def _llm_rerank(self, query: str, candidates: List[Dict], top_k: int = 20) -> List[Dict]:
        """
        阶段2：LLM 精排序
        """
        # 构建候选歌曲上下文
        songs_context = self._build_songs_context(candidates)

        # 构建 LLM Prompt
        prompt = f"""用户查询："{query}"

候选歌曲（{len(candidates)}首）：
{songs_context}

请从候选歌曲中选择最匹配用户查询的歌曲，返回 JSON 格式：
{{
  "matched_songs": [
    {{
      "song_id": 123,
      "match_score": 0.95,
      "match_reason": "匹配原因说明"
    }}
  ]
}}

要求：
1. 理解用户查询中的所有要素（风格、情绪、乐器、场景等）
2. 选择最符合用户需求的歌曲
3. match_score 范围：0.6-1.0，低于0.6的不返回
4. 最多返回 {top_k} 首歌曲
5. 提供简洁的匹配理由（20字以内）
"""

        # 调用 LLM
        try:
            response = self.llm.chat([{"role": "user", "content": prompt}])
            result = json.loads(response)
            matched_songs = result.get('matched_songs', [])

            # 补充完整信息
            final_results = []
            for matched in matched_songs:
                song_id = matched['song_id']
                # 从候选中找到完整信息
                for candidate in candidates:
                    if candidate['song_id'] == song_id:
                        final_results.append({
                            'song_id': song_id,
                            'song_name': candidate['song_name'],
                            'artist': candidate['artist'],
                            'album': candidate['album'],
                            'match_score': matched['match_score'],
                            'match_reason': matched.get('match_reason', '')
                        })
                        break

            print(f"LLM 精排: {len(final_results)} 首最终结果")
            return final_results

        except Exception as e:
            print(f"LLM 精排失败: {e}")
            # 降级：返回向量召回的 top_k
            return candidates[:top_k]

    def _build_songs_context(self, candidates: List[Dict]) -> str:
        """构建候选歌曲的上下文文本"""
        context_lines = []
        for i, song in enumerate(candidates, 1):
            line = f"{i}. 歌名：{song['song_name']}, 艺术家：{song['artist']}"
            if song.get('genre'):
                line += f", 风格：{song['genre']}"
            if song.get('mood'):
                line += f", 情绪：{song['mood']}"
            if song.get('scene'):
                line += f", 场景：{song['scene']}"
            context_lines.append(line)

        return '\n'.join(context_lines)

    def _save_history(
        self,
        session_id: Optional[str],
        user_id: Optional[str],
        query_text: str,
        result_count: int,
        latency_ms: int
    ) -> int:
        """保存推荐历史"""
        cursor = self.db.cursor()
        query = """
            INSERT INTO recommendation_history (
                session_id, user_id, query_text, query_type,
                result_count, latency_ms
            ) VALUES (%s, %s, %s, %s, %s, %s)
            RETURNING id
        """
        cursor.execute(query, (
            session_id, user_id, query_text, 'complex',
            result_count, latency_ms
        ))
        history_id = cursor.fetchone()[0]
        self.db.commit()
        cursor.close()
        return history_id
```

---

## 阶段4：API 测试与调优（预计工作量：1天）

### 4.1 端到端测试

创建测试脚本 `/home/luke/code_project/musicRecommend/tests/test_recommend_e2e.py`

```python
#!/usr/bin/env python3
import requests
import json

BASE_URL = 'http://localhost:5000'

def test_recommend():
    """测试推荐接口"""
    print("=" * 60)
    print("测试1：复杂查询 - 欢快的凯尔特音乐，有小提琴伴奏")
    print("=" * 60)

    response = requests.post(
        f'{BASE_URL}/api/recommend',
        json={
            'query': '欢快的凯尔特音乐，有小提琴伴奏',
            'session_id': 'test_001',
            'max_results': 20
        }
    )

    print(f"状态码: {response.status_code}")
    result = response.json()
    print(f"成功: {result.get('success')}")
    print(f"结果数量: {result.get('total')}")
    print(f"响应时间: {result.get('latency_ms')} ms")

    if result.get('results'):
        print("\n前3个结果:")
        for i, song in enumerate(result['results'][:3], 1):
            print(f"\n{i}. {song['song_name']} - {song['artist']}")
            print(f"   匹配度: {song.get('match_score', 0)}")
            print(f"   理由: {song.get('match_reason', '')}")

    print("\n" + "=" * 60)
    print("测试2：简单查询 - 适合工作的轻音乐")
    print("=" * 60)

    response = requests.post(
        f'{BASE_URL}/api/recommend',
        json={
            'query': '适合工作的轻音乐',
            'session_id': 'test_001'
        }
    )

    result = response.json()
    print(f"结果数量: {result.get('total')}")
    print(f"响应时间: {result.get('latency_ms')} ms")

if __name__ == '__main__':
    test_recommend()
```

运行测试：
```bash
# 确保后端服务已启动
cd ~/code_project/music-project/backend
~/miniconda3/envs/music/bin/python app.py &

# 运行测试
~/miniconda3/envs/music/bin/python /home/luke/code_project/musicRecommend/tests/test_recommend_e2e.py
```

---

### 4.2 性能基准测试

```python
#!/usr/bin/env python3
import requests
import time
import statistics

def benchmark_recommend():
    """性能基准测试"""
    test_queries = [
        "欢快的凯尔特音乐，有小提琴伴奏",
        "适合工作的轻音乐",
        "悲伤的钢琴曲",
        "激昂的摇滚音乐"
    ]

    latencies = []

    for query in test_queries:
        start = time.time()
        response = requests.post(
            'http://localhost:5000/api/recommend',
            json={'query': query, 'session_id': 'benchmark'}
        )
        latency = (time.time() - start) * 1000
        latencies.append(latency)

        result = response.json()
        print(f"查询: {query}")
        print(f"  响应时间: {latency:.0f} ms")
        print(f"  结果数量: {result.get('total', 0)}\n")

    print("=" * 60)
    print("性能统计:")
    print(f"  平均响应时间: {statistics.mean(latencies):.0f} ms")
    print(f"  最小响应时间: {min(latencies):.0f} ms")
    print(f"  最大响应时间: {max(latencies):.0f} ms")
    print(f"  标准差: {statistics.stdev(latencies):.0f} ms")

if __name__ == '__main__':
    benchmark_recommend()
```

**性能目标**：
- 向量召回：< 100 ms
- LLM 精排：< 2000 ms
- 总响应时间：< 2500 ms

运行测试：
```bash
# 运行基准测试
~/miniconda3/envs/music/bin/python /home/luke/code_project/musicRecommend/tests/test_benchmark.py
```

---

### 4.1/4.2 关键参数说明

两阶段推荐系统涉及以下关键参数：

#### 阶段1：向量召回参数

| 参数 | 默认值 | 说明 | 修改位置 |
|------|--------|------|----------|
| `recall_size` | 15 | 向量召回的候选歌曲数量 | `recommend_service.py:33` `_vector_recall()` |
| `music_file` 过滤 | 启用 | 只返回有实际音乐文件的歌曲 | `recommend_service.py:98-99` |

```python
# 向量召回 - 建议值 10-50
candidates = self._vector_recall(query, recall_size=15)

# music_file 过滤条件（非空字符串）
WHERE se.embedding IS NOT NULL
  AND s.music_file IS NOT NULL
  AND s.music_file != ''
```

#### 阶段2：LLM 精排参数

| 参数 | 默认值 | 说明 | 修改位置 |
|------|--------|------|----------|
| `max_results` / `top_k` | 20 | 最终返回给用户的歌曲数量 | `routes/recommend.py:32` / `recommend_service.py` |

#### Embedding 模型配置

| 配置项 | 当前值 | 说明 | 修改位置 |
|--------|--------|------|----------|
| `EMBEDDING_USE_LOCAL` | `true` | 使用本地模型还是 API | `.env` |
| `LOCAL_EMBEDDING_MODEL` | `BAAI/bge-m3` | 本地模型名称 | `.env` |
| `GPU_GROUP` | `0` | 使用的 GPU 编号 | `.env` |

```bash
# 本地模式（当前配置）- 使用本地 GPU
EMBEDDING_USE_LOCAL=true

# API 模式 - 使用外部 Embedding API
EMBEDDING_USE_LOCAL=false
EMBEDDING_API_KEY=your_key
```

#### 性能参考

| 阶段 | 耗时 | 占比 | 说明 |
|------|------|------|------|
| 向量召回 | ~5-10s | 10-20% | embedding 生成 + pgvector 查询 |
| LLM 精排 | ~15-40s | 80-90% | 外部 API 调用耗时 |

> **注意**：LLM 精排时间主要由外部 API 决定，当前使用 MiniMax API，响应时间约 15-40 秒。

---

### 4.3 结果质量验证

创建质量验证脚本：

```python
#!/usr/bin/env python3
import requests

def validate_quality():
    """验证推荐结果质量"""
    test_cases = [
        {
            'query': '欢快的凯尔特音乐，有小提琴伴奏',
            'expected_keywords': ['凯尔特', 'Celtic', '小提琴', 'violin', '欢快'],
            'description': '复杂查询：多个条件组合'
        },
        {
            'query': '适合工作的背景音乐',
            'expected_keywords': ['工作', '轻音乐', '背景', '轻快', '舒缓'],
            'description': '场景查询：工作场景'
        },
        {
            'query': '悲伤的钢琴曲',
            'expected_keywords': ['钢琴', 'piano', '悲伤', '忧伤', '伤感'],
            'description': '情绪+乐器查询'
        }
    ]

    for test in test_cases:
        print(f"\n{'='*60}")
        print(f"测试: {test['description']}")
        print(f"查询: {test['query']}")
        print(f"{'='*60}")

        response = requests.post(
            'http://localhost:5000/api/recommend',
            json={'query': test['query'], 'session_id': 'quality_test'}
        )

        result = response.json()

        if not result.get('results'):
            print("❌ 无结果返回")
            continue

        # 检查前5个结果
        matched_keywords = set()
        for song in result['results'][:5]:
            song_text = (
                song.get('song_name', '') + ' ' +
                song.get('artist', '') + ' ' +
                song.get('match_reason', '')
            ).lower()

            for keyword in test['expected_keywords']:
                if keyword.lower() in song_text:
                    matched_keywords.add(keyword)

        coverage = len(matched_keywords) / len(test['expected_keywords']) * 100
        print(f"\n关键词覆盖率: {coverage:.0f}%")
        print(f"匹配关键词: {matched_keywords}")
        print(f"期望关键词: {set(test['expected_keywords'])}")

        if coverage >= 40:  # 至少40%的关键词匹配
            print("✅ 质量合格")
        else:
            print("⚠️ 质量有待提升")

if __name__ == '__main__':
    validate_quality()
```

---

## 阶段5：前端集成（可选，如已完成可跳过）

### 5.1 前端调用示例

```javascript
// src/api/index.ts

// 调用推荐接口
export const recommend = (query, sessionId) => {
  return request({
    url: '/recommend',
    method: 'POST',
    data: {
      query: query,
      session_id: sessionId,
      max_results: 20
    }
  })
}

// 使用示例
import { recommend } from '@/api/index'

const query = '欢快的凯尔特音乐，有小提琴伴奏'
const sessionId = 'user_123_session_abc'

recommend(query, sessionId).then(res => {
  if (res.success) {
    console.log('推荐结果数量:', res.total)
    console.log('响应时间:', res.latency_ms, 'ms')
    console.log('推荐歌曲:', res.results)
  }
})
```

---

## 常见问题排查

### Q1: pgvector 扩展安装失败
```bash
# 检查 PostgreSQL 版本
psql --version

# 确保版本 >= 11，推荐 >= 12
# 从源码编译安装
cd /tmp
git clone https://github.com/pgvector/pgvector.git
cd pgvector
make
sudo make install

# 重启 PostgreSQL
sudo systemctl restart postgresql
```

### Q2: Embedding API 调用失败
```python
# 测试 API 连通性
from openai import OpenAI

client = OpenAI(
    api_key="your_key",
    base_url="https://api.minimax.chat"
)

try:
    response = client.embeddings.create(
        model="embo-01",
        input="测试文本"
    )
    print("✅ API 连通正常")
    print(f"向量维度: {len(response.data[0].embedding)}")
except Exception as e:
    print(f"❌ API 调用失败: {e}")
```

### Q3: 向量检索返回结果为空
```sql
-- 检查向量数据
SELECT COUNT(*) as total,
       COUNT(embedding) as with_embedding
FROM song_embeddings;

-- 检查索引
\d song_embeddings

-- 测试向量检索语法
SELECT song_id, song_name,
       1 - (embedding <=> '[0,0,0,...]'::vector) as similarity
FROM song_embeddings
WHERE embedding IS NOT NULL
ORDER BY embedding <=> '[0,0,0,...]'::vector
LIMIT 10;
```

### Q4: LLM 精排超时
```python
# 减少候选数量
candidates = self._vector_recall(query, recall_size=100)  # 从200改为100

# 或者跳过LLM精排，直接返回向量结果
if len(candidates) > 50:
    # 直接返回向量召回结果
    return candidates[:max_results]
else:
    # 候选较少时才用LLM精排
    return self._llm_rerank(query, candidates, top_k=max_results)
```

---

## 完成检查清单

- [ ] pgvector 扩展已安装并启用
- [ ] song_embeddings 表已创建，包含 HNSW 索引
- [ ] 向量数据已生成（至少12万首有歌单的歌曲）
- [ ] Embedding API 测试通过
- [ ] RecommendService 实现了两阶段推荐
- [ ] 端到端测试通过
- [ ] 性能测试达标（<2.5秒）
- [ ] 结果质量验证合格（关键词覆盖率>40%）
- [ ] 前端集成测试通过（如需要）

---

## 下一步优化方向

1. **向量召回优化**
   - 调整 recall_size（100-500之间）
   - 添加标签过滤（genre、mood、scene）
   - 尝试不同的距离度量（cosine、L2、inner product）

2. **LLM 精排优化**
   - 优化 Prompt 设计
   - 调整 temperature 参数
   - 缓存相似查询的结果

3. **性能优化**
   - 使用 Redis 缓存热门查询
   - 批量查询优化
   - 异步处理非关键路径

4. **结果质量优化**
   - A/B 测试不同召回策略
   - 收集用户反馈优化排序
   - 引入多样性（避免结果过于相似）
