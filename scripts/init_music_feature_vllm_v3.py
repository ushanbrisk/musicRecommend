...
VLLM_MODELS = [get_vllm_model(ep) for ep in VLLM_ENDPOINTS]

# 批量配置 - 支持动态调整
# 初始 batch_size，可以根据内容长度分档
...
async def process_group_async(group: int, endpoint: str, model_name: str, total_to_process: int):
    """
    异步处理指定分组的歌曲（从公共池按内容长度排序获取）

    每组从同一个排序好的池子中获取数据，通过 song_id % 3 分发到不同组
    """
    conn = get_db_connection()
    cursor = conn.cursor()

    group_name = chr(ord('A') + group)
    print(f"\n{'='*60}")
    ...
    async with aiohttp.ClientSession() as session:
        while True:
            # 获取待处理歌曲（按内容长度排序）
            songs = get_songs_without_features(cursor, limit=current_batch_size * 10)
            if not songs:
                break

            # 分发给当前组的歌曲 (song_id % 3 == group)
            group_songs = [s for s in songs if int(s['song_id']) % 3 == group]

            if not group_songs:
                continue

            print(f"\n  批次开始 (组 {group_name} 获取 {len(group_songs)} 首, batch_size={current_batch_size})...")

            # 分批调用 vLLM
            for i in range(0, len(group_songs), current_batch_size):
                batch = group_songs[i:i+current_batch_size]
                ...
                try:
                    ...
                    if features_list:
                        ...
                        # 如果连续成功，可以尝试增加 batch_size
                        if success_count > 0 and success_count % (current_batch_size * 5) == 0:
                            if current_batch_size < MAX_BATCH_SIZE:
                                current_batch_size = min(current_batch_size + 1, MAX_BATCH_SIZE)
                                print(f"      [自适应] batch_size 调整为 {current_batch_size}")
                    else:
...
async def init_music_feature_vllm():
    """
    使用 vLLM 批量生成音乐特征（3组并行版本）

    3 组并行处理，按 song_id % 3 分发到不同组
    歌曲按内容长度排序，短内容优先处理
    支持自适应 batch_size 调整
    """
    print("=" * 60)
    print("vLLM 批量特征生成开始 (3组并行 TP=2)")
    print("=" * 60)
    print(f"  初始 batch_size: {INITIAL_BATCH_SIZE}")
    print(f"  最大 batch_size: {MAX_BATCH_SIZE}")
    print(f"  端点: {VLLM_ENDPOINTS}")
    print(f"  模型: {VLLM_MODELS}")
    print()
    ...
    try:
        ...
        print(f"      (3组并行处理，每组约 {total_to_process//3:,} 首)")
        ...
        # 3. 异步并行处理 3 组
        print("\n[3/4] 启动 vLLM 批量处理 (3组并行)...")

        start_time = time.time()

        # 并行处理 3 组
        results = await asyncio.gather(
            process_group_async(0, VLLM_ENDPOINTS[0], VLLM_MODELS[0], total_to_process // 3 + 1),
            process_group_async(1, VLLM_ENDPOINTS[1], VLLM_MODELS[1], total_to_process // 3 + 1),
            process_group_async(2, VLLM_ENDPOINTS[2], VLLM_MODELS[2], total_to_process // 3 + 1),
        )
