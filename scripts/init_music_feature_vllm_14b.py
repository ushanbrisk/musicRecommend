...
"""
============================================================
vLLM 批量特征生成脚本 - Qwen2.5-14B 对比实验
============================================================

用于与 Qwen2.5-32B 进行对比实验，评估 14B 模型的效果。

## 与 32B 版本 (init_music_feature_vllm_v2.py) 的差异

- 特征存储表: music_features_14b (而非 music_features)
- 跳过记录表: music_features_14b_skip (而非 music_features_skip)
- 独立运行，不影响 32B 的已有进度
...
2. 运行特征生成:
   ~/miniconda3/envs/music_recommend/bin/python scripts/init_music_feature_vllm_14b.py
...
def ensure_skip_table_exists(conn):
    ...
    cursor = conn.cursor()
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS music_features_14b_skip (
...
def get_songs_without_features(cursor, limit: int = 100) -> List[Dict]:
    ...
    query = """
        ...
        LEFT JOIN music_features_14b mf ON s.song_id = mf.song_id
        LEFT JOIN music_features_14b_skip sk ON s.song_id::text = sk.song_id
        WHERE mf.id IS NULL
...
def mark_song_skipped(conn, song_id: int, fail_reason: str):
    ...
    cursor = conn.cursor()
    cursor.execute("""
        INSERT INTO music_features_14b_skip (song_id, fail_reason, fail_count, updated_at)
        VALUES (%s, %s, 1, CURRENT_TIMESTAMP)
        ON CONFLICT (song_id) DO UPDATE SET
            fail_reason = EXCLUDED.fail_reason,
            fail_count = music_features_14b_skip.fail_count + 1,
...
def save_features(features_list: List[Dict], cursor, conn):
    ...
    for features in features_list:
        try:
            ...
            query = """
                INSERT INTO music_features_14b (
...
async def init_music_feature_vllm():
    ...
    print("=" * 60)
    print("vLLM 批量特征生成开始 (Qwen2.5-14B 对比实验, 动态 batch_size)")
    print("=" * 60)
    print(f"  初始 batch_size: {INITIAL_BATCH_SIZE}")
    ...
    try:
        ...
        cursor.execute("SELECT COUNT(*) FROM music_features_14b_skip")
        skip_count = cursor.fetchone()[0]
        print(f"      music_features_14b_skip: {skip_count:,} 条 (已跳过的歌曲)")
        ...
        print(f"  - 成功:   {success_count:,}")
        print(f"  - 失败:   {fail_count:,}")
        print(f"  - music_features_14b 表: {total_features:,}")
