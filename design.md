...
## ⚠️ 6. 实现步骤（详细）—— 已过时，请参考 [第6部分_最新实现步骤.md](./第6部分_最新实现步骤.md)

> **状态**：本章节为早期设计文档，内容已过时
>
> **最新实现参考**：[第6部分_最新实现步骤.md](./第6部分_最新实现步骤.md)
>
> **说明**：实际实现已完成，代码和参数可能与本设计文档有差异，请以此文件为准。
...
- **设计理念**：聊天窗口风格，类似微信/飞书，输入区固定底部
- **技术栈**：Vue 3 Composition API + Pinia + UniApp + SCSS
- **组件设计**：推荐卡片、推荐弹窗、聊天消息列表

#### 新增/修改文件清单

| 类型 | 文件路径 | 说明 |
|------|----------|------|
| 新增 | `src/api/recommend.ts` | 推荐 API 封装 |
| 新增 | `src/stores/recommend.ts` | 推荐状态管理 |
| 新增 | `src/components/RecommendCard.vue` | 推荐结果卡片组件 |
| 新增 | `src/pages/recommend/recommend.vue` | AI 推荐页面（聊天风格） |
| 修改 | `src/pages.json` | 添加推荐页面路由 |
| 修改 | `src/pages/index/index.vue` | 添加"AI推荐"导航入口 |

#### 详细代码

> 详细内容见 `frontend_design.md`

#### 6.5.3-6.5.8 详细代码
> 已移除，详见 `frontend_design.md`
...
| 测试项 | 结果 | 说明 |
|--------|------|------|
| 单元测试（7项） | ✅ 全部通过 | pytest 测试通过 |
| 推荐接口 | ✅ 正常 | 返回 success:true，history_id |
| 古诗词推荐 | ✅ 正常 | "琵琶行" query_type=complex |
| 反馈接口 | ✅ 正常 | ON CONFLICT 更新机制工作正常 |
| 历史查询 | ✅ 正常 | 同一 session_id 返回多条记录 |
| 健康检查 | ✅ 正常 | GET / 返回 version:1.0.0 |
...
|------|------|----------|
| H5 | ✅ 完成 | `dist/build/h5` |
| 微信小程序 | ✅ 完成 | `dist/build/mp-weixin` |

**部署说明**：

- **后端服务**：`music-project/backend/music-recommend.service` systemd 配置文件已准备好
- **Gunicorn**：已安装，可通过 `gunicorn -w 4 -b 0.0.0.0:5000 app:app` 启动
- **前端 H5**：`music-project-uniapp/dist/build/h5` 目录可部署至 Web 服务器
- **微信小程序**：使用微信开发者工具导入 `music-project-uniapp/dist/build/mp-weixin`
